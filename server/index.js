...
app.post("/api/contact", async (req, res) => {
  ...
  try {
    // Create transporter (explicitly configured for Gmail SMTP)
    ...
    // Email Options
    const mailOptions = {
      from: `"${name}" <${emailUser}>`, // Must be EMAIL_USER for Gmail auth, but label name as sender
      to: receiverEmail,
      replyTo: email, // Direct replies back to the user's input email
      ...
      html: `
        <div style="font-family: Arial, sans-serif; padding: 20px; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px;">
          ...
          <p style="font-size: 11px; color: #9ca3af; margin-top: 30px; border-top: 1px solid #eef2f6; padding-top: 10px; text-align: center;">
            Sent from Noraiz Amaan's Portfolio contact backend.
          </p>
        </div>
      `
    };
